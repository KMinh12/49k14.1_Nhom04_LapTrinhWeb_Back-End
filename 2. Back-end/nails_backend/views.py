...
def CapNhatLich(request, booking_id):
    if request.method == 'POST':
        ...
        try:
            ...
            # 1. HỖ TRỢ CẢ DỮ LIỆU JSON (FETCH) VÀ FORM-DATA
            ...
            # Kiểm tra xem có gửi thiếu dữ liệu không
            ...
            # 4. KIỂM TRA TRÙNG LỊCH (Trừ chính cái lịch đang sửa ra)
            overlapping_bookings = Booking.objects.filter(
                employee=new_employee,
                booking_date=parsed_date,
                status__in=['PENDING', 'CONFIRMED']
            ).exclude(id=booking.id).filter( # Bỏ qua lịch hiện tại đang sửa
            ...
            # In chi tiết lỗi ra màn hình Terminal để dễ debug
...
def QuanLyTaiKhoan_KH(request):
    ...
    return render(request, 'KhachHang/QuanLyTaiKhoan/TaiKhoan_KH.html')
...
class EmployeeViewSet(viewsets.ModelViewSet):
    ...
    # Kế thừa API xem lịch trống từ class thứ 1
